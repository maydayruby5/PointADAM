...
from pointclip.co_attention import Self_Attention
from pointclip.fewshot import PointCLIP_Model, load_clip_to_cpu
K = 20
...
class DGCLIP(nn.Module):
    ...
    def forward(self, x, activate_DefRec=False):
        ...
        x0 = get_graph_feature(x, self.args, k=self.k)# x[8,3,1024] x0[8,6,1024,20] 20个邻居
        transformd_x0 = self.input_transform_net(x0)# [8,3,3]
        x = torch.matmul(transformd_x0, x)# [8,3,1024]

        x = get_graph_feature(x, self.args, k=self.k)# [8,6,1024,20]
        x = self.conv1(x)# [8,64,1024,20]
        x1 = x.max(dim=-1, keepdim=False)[0]# [8,64,1024]

        x = get_graph_feature(x1, self.args, k=self.k)# [8,128,1024,20]
        x = self.conv2(x)# [8,64,1024,20]
        x2 = x.max(dim=-1, keepdim=False)[0]# [8,64,1024]

        x = get_graph_feature(x2, self.args, k=self.k)# [8,128,1024,20]
        x = self.conv3(x)# [8,128,1024,20]
        x3 = x.max(dim=-1, keepdim=False)[0]# [8,128,1024]

        x = get_graph_feature(x3, self.args, k=self.k)# [8,256,1024,20]
        x = self.conv4(x)# [8,256,1024,20]
        x4 = x.max(dim=-1, keepdim=False)[0]# [8,256,1024]

        x_cat = torch.cat((x1, x2, x3, x4), dim=1)# [8,512,1024]
