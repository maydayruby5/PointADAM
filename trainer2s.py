...
from PointDA.Models import PointNet
from PointDA.Models import Discriminator
...
parser.add_argument('--exp_name', type=str, default='shapenettomodelnetadwoat',  help='Name of the experiment')
parser.add_argument('--out_path', type=str, default='./experiments', help='log folder path')
parser.add_argument('--dataroot', type=str, default='./data', metavar='N', help='data path')
parser.add_argument('--src_dataset', type=str, default='shapenet', choices=['scannet', 'c', 'scannet'])
parser.add_argument('--trgt_dataset', type=str, default='modelnet', choices=['c', 'shapenet', 'scannet'])
...
for epoch in range(args.epochs):
    ...
    src_print_losses = {"total": 0.0, cls_type: 0.0, "cls": 0.0}

    if args.DefRec_on_src:
        src_print_losses['DefRec'] = 0.0

    trgt_print_losses = {'DefRec': 0.0, 'adv': 0.0}
    ...
    for data1, data2 in zip(src_train_loader, trgt_train_loader):
        ...
        if data1 is not None:
            ...
            if args.DefRec_on_src:
                ...
                src_logits,_ = model(src_data, activate_DefRec=True)
            ...
            if args.apply_PCM:
                ...
                src_cls_logits, xs = model(src_data, activate_DefRec=False)
        ...
        if data2 is not None:
            ...
            defrec_loss =  trgt_defrec_loss

            discriminator_out = ad_net(torch.cat((src_fea, target_fea), 0))
            adloss = AdLoss(discriminator_out, criterion, args.batch_size)

            trgt_print_losses['adv'] += adloss.item() * batch_size * 2
            trgt_print_losses['DefRec'] += defrec_loss.item() * batch_size * 2
            src_print_losses['total'] += defrec_loss.item() * batch_size * 2 + adloss.item() * batch_size * 2

            loss = defrec_loss + 0.01*adloss * args.adv_weight
